...
const MainPage = () => {
    ...
    const { setLenghtTasks } = useContext(Context);

    // Array for Tasks
    const [tasksBacklog, setTasksBacklog] = useState(() => JSON.parse(localStorage.getItem('backlog')) || []);
    const [tasksReady, setTasksReady] = useState(() => JSON.parse(localStorage.getItem('ready')) || []);
    const [tasksInPropgress, setTasksInPropgress] = useState(() => JSON.parse(localStorage.getItem('in progress')) || []);
    const [tasksFinished, setTasksFinished] = useState(() => JSON.parse(localStorage.getItem('finished')) || []);
    ...
    }, [tasksBacklog, tasksFinished, setLenghtTasks]);
    ...
    function submitNewTaskBacklog() {
        if (inputvalue !== "") {
            const newArray = getNewArrayBlock(tasksBacklog, inputvalue, "backlog");
            setTasksBacklog(newArray);
            localStorage.setItem('backlog', JSON.stringify(newArray));
        }
        setInputValue('');
        setInputActive(!inputActive);
        setSubmitButton(!submitButton);
    ...
    // const deleteAllTask = () => {
    //     const storageKeys = ['backlog', 'ready', 'in progress', 'finished'];
